...
class Stack:
    ...
    # Add an element (word) to the top of the stack
    def push(self, value):
        new_node = Node(value)
    ...
    # Remove and return the top element (word)
    def pop(self):
        if self.isEmpty():
    ...
    # Display all elements (words) in the stack from top to bottom
...
# Create a stack and test the methods with words
myStack = Stack()

# Test pushing words onto the stack
myStack.push('Afaq')
myStack.push('Zeeshan')
myStack.push('Minaahil')
myStack.push('Minhal')
